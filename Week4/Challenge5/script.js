function generate_board() {
    ...
    // YOUR CODE GOES HERE
    // Build an HTML table grid with 4 columns and N rows.
    // IMPORTANT: Start every card hidden using cards/hidden.png
    let result_str = `
            ...
            result_str += `
                <td style="text-align:center;">
                    <img 
                        ...
                        alt="Hidden card"
                        data-card="${filename}"
                        style="width:150px;height:auto;display:block;cursor:pointer;"
                    >
                </td>
            `;
        }
    ...
    let score = 0;
    totalHeader.textContent = `Total Score: ${score}`;
    let flipped = [];          // currently flipped but not yet resolved
    let lock = false;          // prevent clicks during mismatch timeout
    const totalPairs = randomizedCards.length / 2;

    const boardContainer = document.getElementById('game-board');

    boardContainer.onclick = function (e) {
        const img = e.target.closest('img.card');
        if (!img) return;
        if (lock) return;
        if (img.classList.contains('matched')) return;
        if (img.classList.contains('flipped')) return;

        // Flip and reveal card
        const face = img.getAttribute('data-card');
        img.src = `cards/${face}`;
        ...
        if (flipped.length === 2) {
            ...
            // Ignore if same element flipped twice
            if (first === second) {
                flipped = [first];
                return;
            }

            const isMatch = first.dataset.card === second.dataset.card;
            ...
            if (isMatch) {
                ...
                if (score === totalPairs) {
                    totalHeader.textContent = 'All Matched, Congratulations!';
                } else {
                    totalHeader.textContent = `Total Score: ${score}`;
                }
            } else {
                ...
                lock = true;
                setTimeout(() => {
                    first.src = 'cards/hidden.png';
                    second.src = 'cards/hidden.png';
